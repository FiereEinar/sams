import type { SidebarLogoType } from '@/hooks/use-theme';
...
export default function SidebarHeader({ title, subtitle, icon, logoType = 'icon', logoImageUrl, isHorizontal }: SidebarHeaderProps) {
  return (
    <div className={`flex items-center gap-3 ${isHorizontal ? 'py-3 pr-6' : 'p-6'}`}>
      {logoType === 'image' && logoImageUrl ? (
        <img
          src={logoImageUrl}
          alt={title}
          className={`rounded-lg object-cover ${isHorizontal ? 'size-8' : 'size-10'}`}
        />
      ) : (
