...
export default function AppearanceSettings() {
  ...
  const isCustomActive = accentColor.name === 'Custom' || !ACCENT_PRESETS.some((p) => p.primary === accentColor.primary);
      ...
      <div className={!canEditAppearance ? 'pointer-events-none opacity-60' : ''}>
        ...
        <section id="settings-appearance-mode">
          ...
          <div className="mt-4 flex items-center gap-2 rounded-xl bg-slate-100 px-4 py-3 text-sm dark:bg-white/5">
            <span className="material-symbols-outlined text-base text-primary">{resolvedDark ? 'dark_mode' : 'light_mode'}</span>
        ...
        <section id="settings-accent-color">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              style={isCustomActive ? { borderColor: accentColor.primary, boxShadow: `0 4px 20px ${accentColor.primary}25` } : {}}
            >
              <div className="relative">
                <div
                  className="flex size-10 items-center justify-center rounded-full transition-transform group-hover:scale-110"
                  style={{
                    background: isCustomActive ? customColor : 'conic-gradient(from 0deg, #f43f5e, #f59e0b, #10b981, #3b82f6, #8b5cf6, #f43f5e)',
                  }}
                >
                  {isCustomActive ? (
                    <span className="material-symbols-outlined text-lg text-white drop-shadow">check</span>
